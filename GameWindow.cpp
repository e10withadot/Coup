...
: QWidget(), players(players), button_event(false) {
    ...
    void (GameWindow::*funcs[12])() = {
        &GameWindow::gatherPress,   &GameWindow::taxPress,
        &GameWindow::arrestPress,   &GameWindow::bribePress,
        &GameWindow::sanctionPress, &GameWindow::coupPress,
        &GameWindow::seeCoinsPress, &GameWindow::blockArrestPress,
        &GameWindow::undoTaxPress,  &GameWindow::undoBribePress,
        &GameWindow::undoCoupPress, &GameWindow::investPress};
    const char *b_name[12] = {
        "Gather - Free",     "Tax - Free",          "Arrest - Free",
        "Bribe - 4 coins",   "Sanction - 3 coins",  "Coup - 7 coins",
        "See Coins - Free",  "Block Arrest - Free", "Undo Tax - Free",
        "Undo Bribe - Free", "Undo Coup - 5 coins", "Invest - 3 coins"};
    ...
    for (int i = 0; i < 12; i++) {
        ...
        if (i != 6)
            connect(button, &QPushButton::clicked, this, &GameWindow::onMove);
...
void GameWindow::refreshButtons() {
    ...
    switch (turn->role()) {
        case SPY:
            ...
            break;
        case GOVERNOR:
            if (CUR_GAME->getLast(TAX) != nullptr)
        ...
        case JUDGE:
            ...
            break;
        case GENERAL:
            if (CUR_GAME->getLast(COUP) != nullptr && turn->coins() >= 5)
...
Player *GameWindow::selectionInterface(Player *turn, const char *msg) {
    ...
    if (turn->cpu()) {
        ...
        targetIndex = possibleTargets[QRandomGenerator::global()->bounded(
...
void GameWindow::seeCoinsPress() {
    ...
    QMessageBox::information(this, "Coins Revealed",
                             QString("Player %1 has %2 coins.")
                             .arg(targetIndex + 1)
                             .arg(players[targetIndex]->coins()));
    gameLoop();
}
void GameWindow::gatherPress() {
    Player *turn = CUR_GAME->turn();
    Action *a = turn->gather();
    sys_l->setText(QString("Player %1 gathered %2 coins.")
                   .arg(turn->index() + 1)
                   .arg(a->coin_change));
}
void GameWindow::taxPress() {
    ...
    sys_l->setText(QString("Player %1 took %2 coins from the pool.")
                   .arg(turn->index() + 1)
                   .arg(a->coin_change));
}
void GameWindow::undoTaxPress() {
    ...
    sys_l->setText(
        QString(
            "Player %1 undoed Player %2's tax action!\nPlayer %2 lost %3 coins.")
        .arg(turn->index() + 1)
        .arg(a->reciever->index() + 1)
        .arg(a->coin_change));
}
void GameWindow::arrestPress() {
    ...
    sys_l->setText(QString("Player %1 arrested Player %2! Player %2 gave %3 "
                           "coin(s) to Player %1.")
                   .arg(turn->index() + 1)
                   .arg(target->index() + 1)
                   .arg(a->coin_change));
...
void GameWindow::blockArrestPress() {
    Spy *turn = (Spy *)(CUR_GAME->turn());
    Player *target =
        selectionInterface(turn, "Select a player to block from doing arrests:");
    Action *a = turn->block_arrest(target);
    sys_l->setText(QString("Player %1 blocked arrest on Player %2!")
                   .arg(turn->index() + 1)
                   .arg(target->index() + 1));
}
...
void GameWindow::bribePress() {
    ...
    sys_l->setText(
        QString("Player %1 did a bribe! They get an additional turn next time!")
        .arg(turn->index() + 1));
}
void GameWindow::undoBribePress() {
    ...
    sys_l->setText(QString("Player %1 undid Player %2's bribe!")
                   .arg(turn->index() + 1)
                   .arg(a->reciever->index() + 1));
}
void GameWindow::sanctionPress() {
    ...
    sys_l->setText(QString("Player %1 sanctioned Player %2!")
                   .arg(turn->index() + 1)
                   .arg(target->index() + 1));
}
void GameWindow::coupPress() {
    Player *turn = CUR_GAME->turn();
    Player *target =
        selectionInterface(turn, "Select a player to throw a coup on:");
    Action *a = turn->coup(target);
    CUR_GAME->setLast(COUP, a);
    sys_l->setText(QString("Player %1 threw a coup on Player %2!")
                   .arg(turn->index() + 1)
                   .arg(target->index() + 1));
}
void GameWindow::undoCoupPress() {
    ...
    sys_l->setText(QString("Player %1 undid the coup on Player %2!")
                   .arg(turn->index() + 1)
                   .arg(a->reciever->index() + 1));
}
void GameWindow::investPress() {
    ...
    sys_l->setText(QString("Player %1 invested 3 coins and got 200% back!")
                   .arg(turn->index() + 1));
}
...
vector<int> GameWindow::allowedButtons(Player *p) {
    ...
    switch (p->role()) {
        ...
        case SPY:
            ...
            break;
        case GENERAL:
            if (p->coins() >= 5 || CUR_GAME->getLast(COUP))
                buttons.push_back(9);
            break;
        case JUDGE:
            if (CUR_GAME->getLast(BRIBE) != nullptr)
...
void GameWindow::gameLoop() {
    ...
    if (turn->cpu()) {
        QTimer::singleShot(2000, this, [this, turn]() {
            ...
            if (!allowed.empty()) {
                int choice = QRandomGenerator::global()->bounded(
